...
class OdometryPublisher(Node):
    ...
    def timer_callback(self):
        ...
        dt = 0.1
        delta_x = (self.odom.twist.twist.linear.x * dt * cos(self.odom.pose.pose.orientation.z))
        delta_y = (self.odom.twist.twist.linear.x * dt * sin(self.odom.pose.pose.orientation.z))
        delta_th = (self.odom.twist.twist.angular.z * dt)
        self.odom.pose.pose.position.x += delta_x
        self.odom.pose.pose.position.y += delta_y
        self.odom.pose.pose.orientation.z += delta_th
        self.publisher_.publish(self.odom)
        self.get_logger().info('Publishing: "%f"' % self.odom.pose.pose.position.x)
        self.get_logger().info('Publishing: "%f"' % self.odom.pose.pose.position.y)
        self.get_logger().info('Publishing: "%f"' % self.odom.pose.pose.orientation.z)
